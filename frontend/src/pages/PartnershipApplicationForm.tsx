...
import { PartnershipApplicationsAPI } from "@/services/application";
import {  mapFormDataToBackend } from '@/lib/mappings';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateStep(11)) {
      try {
        ...
        await PartnershipApplicationsAPI.create(mappedData);

      console.log("Submission successful:", mappedData);
              ...
              <div className="flex items-center justify-center gap-2 mb-8 overflow-x-auto">
                {steps.map((step, index) => (
                  <div key={step.id} className="flex items-center gap-2 min-w-fit">
                    ...
                    <span className={`font-medium transition-colors text-sm ${currentStep === step.id
                    ...
                    {index < steps.length - 1 && (
                      <div className={`w-4 h-0.5 transition-colors ${currentStep > step.id ? 'bg-green-500' : 'bg-gray-300'
                        }`} />
                    )}
                  </div>
                ))}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {nonCashContributions.map((contribution) => (
                            <Button
                              ...
                              className={`${formData.nonCash.includes(contribution)
